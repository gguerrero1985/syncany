 ...
 *
 * <p>
 * By delaying the events for a few milliseconds, it can match rename from/to
 * events and hence fire artificial an artificial ENTRY_RENAME_FROM_TO event. It
 * furthermore delays ENTRY_CREATE and ENTRY_MODIFY events until the respective
 * files do not change anymore (esp. important for large files).
 *
 * <p>
 * TODO - copying a folder does not fire the new child events - allow flag "fire
 * child events" (on rename, or copy) - allow own user Kind[] array - file
...
public class BufferedWatcher {
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                try {
                    ...
                    logger.log(Level.WARNING, config.getMachineName() + "#Error while processing delayed events. IGNORING.", e);
                }
            }
        }, 0, delay);
    }
        ...
        synchronized (eventQueue) {
            while (eventQueue.size() > 0) {
                ...
                if (e.getEvent().kind() == StandardWatchEventKind.ENTRY_CREATE
                    ...
                    if (file == null) {
                        logger.log(Level.WARNING, "{0}#Purging delayed event. Watch key invalid. File vanished? Event = {1}, Key ={2}", new Object[]{config.getMachineName(), e.getEvent(), e.getParentKey()});
                    }
                ...
                if (e.getEvent().kind() == ExtendedWatchEventKind.ENTRY_RENAME_FROM) {
                    ...
                    cookieSearch:
                    for (TimedWatchEvent potentialMovedToEvent : eventQueue) {
    ...
    private synchronized void removeWatch(WatchNode node) {
        if (node == null) {
            logger.log(Level.WARNING, "{0}#Cannot remove watch. Node is null.", config.getMachineName());
            return;
        }
    ...
    private void processEvent(ExtendedWatchEvent xe) {
        ...
        if (parentNode == null || parentKey == null || event == null) {
            logger.log(Level.WARNING, "{0}#Cannot process event{1}. Invalid values: parentNode = {2}, parentKey = {3}, event = {4}", new Object[]{config.getMachineName(), xe, parentNode, parentKey, event});
            return;
        }
        ...
        if (event.kind() == StandardWatchEventKind.ENTRY_CREATE
            ...
            logger.log(Level.INFO, "{0}#{1} {2}", new Object[]{config.getMachineName(), event.kind().name(), file});
            ...
            if (file.isDirectory() && parentNode.isRecursive()
                ...
                try {
                    ...
                    //checkthis
                    logger.log(Level.SEVERE, config.getMachineName() + "#Could not add log to " + file + ". IGNORING.", ex);
                }
            }
            ...
            logger.log(Level.INFO, "{0}#{1} {2} -> {3}", new Object[]{config.getMachineName(), event.kind().name(), fromFile, toFile});
    ...
    private synchronized void updateMoveFileMaps(File fromFile, WatchKey toParentKey, File toFile) {
        logger.log(Level.CONFIG, config.getMachineName() + "#updating file maps : from " + fromFile + " to " + toFile);

        // Update given key
        WatchNode moveNode = fileNodeMap.get(fromFile);

        if (moveNode == null) {
            logger.log(Level.WARNING, config.getMachineName() + "WatchKey for file {0} not found. IGNORING.", fromFile);
            return;
        }
        ...
        public void run() {
            while (true) {
                ...
                try {
                    ...
                    logger.log(Level.CONFIG, "{0}#watch service closed, terminating.", config.getMachineName());
                    break;
